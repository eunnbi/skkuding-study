...
const main = document.querySelector("main")!; // null 단언문
/* 영화 리스트 렌더링 */
// 타입 별칭
type MovieList = {
  adult: boolean;
...
const renderMovieList = (movieList: MovieList) => { // parameter 타입 선언
  main.innerHTML = "";
  movieList.forEach((movie) => {
    const movieCard = document.createElement("a"); // const으로 선언한 변수의 타입 추론
...
const searchInput = document.querySelector<HTMLInputElement>("input.search-input")!; // 제네릭 타입 통해 반환 타입 설정
...
searchInput.addEventListener("input", (e) => {
  const target = e.currentTarget as HTMLInputElement; // 타입 단언문
